...
const firstPosition = sentence.indexOf('because');
const lastPosition = sentence.lastIndexOf('because');
console.log(sentence.substr(firstPosition, lastPosition)); // ????
...
const incomeString =
  'He earns 5000,03 euro from salary per month, 10000 euro annual bonus, 15000 euro online courses per month.';
console.log(incomeString);
const incomeNumber = incomeString.match(/[0-9]+/g);
console.log(
  `Total annual: ${parseInt(incomeNumber[0]) * 12 +
    parseInt(incomeNumber[1]) +
    parseInt(incomeNumber[2] * 12)}`
);

const income = incomeString.split(' ');
const number = [];
for (let i = 0; i < income.length; i++) {
  // if (parseFloat(income[i])) {
  //   number.push(parseFloat(income[i]));
  // }
  console.log(typeof income[i]);
...
console.log(`Total annual: ${number[0] * 12 + number[1] + number[2] * 12}`);
